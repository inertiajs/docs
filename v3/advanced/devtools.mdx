...
A request made from inside a [layer](/v3/the-basics/manual-visits#layers) is badged with the layer it targets, and a response that opens a layer, [closes one](/v3/the-basics/responses#closing-a-layer), or takes a [detour](/v3/the-basics/redirects#detours) is badged as such. Once a layer has been seen, the filters gain a layer option, so you may narrow the timeline to the base page or to a single layer, which is useful when a wizard steps through several URLs inside one.
...
When layers are open, the tab also lists them beneath the page's props, bottom of the stack first, each with its component, URL, key, base, props, and flash data. The layer the selected request landed on is highlighted, and each layer links to the request that last opened or rewrote it, so that layer's own props, route, and HTTP details are one click away.

## Client-Side Events

Some interactions never reach the server, so the Network tab shows nothing for them. The extension records them as their own timeline entries: [Client Side Visits](/v3/the-basics/manual-visits#client-side-visits), requests served from the [prefetch](/v3/data-props/prefetching) cache, a [local layer](/v3/the-basics/manual-visits#local-layers) opening, a layer closing, and a [layer event](/v3/the-basics/manual-visits#layer-events) being emitted. An event entry names the layer that emitted it and the one that received it, and shows the payload in its Props tab.
...
DevTools consists of two pieces that split the work across each request. The Laravel adapter records the server side as the response is prepared, while the browser extension captures the client side, and the two are paired into a single timeline entry. They communicate over a [documented protocol](/v3/advanced/devtools-protocol) that any server adapter may implement, with the Laravel adapter serving as the reference implementation. Entries are stored locally over the same origin, so nothing is sent to a remote service and no account or cloud sync is required.
