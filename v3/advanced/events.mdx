...
When the visit was issued from inside a [layer](/v3/the-basics/manual-visits#requests-inside-a-layer), `event.detail.page` is that layer's own page, not the page beneath it. The full stack is available as `event.detail.stack`.
...
As with `success`, a visit issued from inside a [layer](/v3/the-basics/manual-visits#requests-inside-a-layer) reports that layer's own page in `event.detail.page`, with the full stack in `event.detail.stack`.
