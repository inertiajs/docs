...
The `key` is the layer's identity. The client defaults it to the `component` name, so a server that is happy with that default omits it rather than repeating it. The `base` is the url of the page the layer is opened over, and is omitted when the layer has no base. A layer that declares neither is sent as `"layer": {}`.
