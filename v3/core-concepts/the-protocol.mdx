...
A **layer** is a page rendered over the page it was opened from, instead of replacing it. The server marks a response as a layer by adding the `layer` field to the [page object](#the-page-object). Both of its members are optional, and the field never appears on an ordinary page response.
...
The `key` is the layer's identity. The client defaults it to the `component` name, so a server that is happy with that default omits it rather than repeating it. The `base` is the url of the page the layer is opened over, sent host-relative the way `url` is, and is omitted when the layer has no base. A layer that declares neither is sent as `"layer": {}`.

A layer response carries its own props, errors, deferred props, and flash data, exactly as any page would.
...
The close page deliberately carries no `flash` key. Flash data stays in the session and is consumed by the refresh that follows. This holds when no layer is open too, so the flash still arrives.
