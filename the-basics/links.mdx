...
[Wayfinder](https://github.com/laravel/wayfinder) in conjunction with the
`Link`component, you can simply pass the resulting object directly to the `href`
prop. The `Link` will infer the HTTP method and URL directly from the Wayfinder
object.
...
<a href="/endpoint" method="post" data={{ foo: bar }}>
  Save
</a>;
```
...
<a href="/endpoint" headers={{ foo: bar }}>
  Save
</a>;
```
...
<a href="/users?active=true" only={["users"]}>
  Show active
</a>;
```
