...
Given the page above, you can render the page by returning an [Inertia response](/responses) from a controller or route. In this example, let's assume this page is stored at <vue>`resources/js/Pages/User/Show.vue`</vue><react>`resources/js/Pages/User/Show.jsx`</react><svelte>`resources/js/Pages/User/Show.svelte`</svelte>within a Laravel application.
