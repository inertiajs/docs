...
The context provides access to all the same properties and methods available through [slot props](#slot-props), including `errors`, `hasErrors`, `processing`, `isDirty`, `submit()`, `reset()`, `setError()`, `clearErrors()`, and Precognition methods when enabled. The hook returns `undefined` when called outside of a `<Form>` component.
