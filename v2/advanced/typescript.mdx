...
You may configure Inertia's types by augmenting the `InertiaConfig` interface in the `@inertiajs/core` module. This can be done by creating an `inertia.d.ts` file in your project's root or `types` directory.
