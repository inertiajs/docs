...
If you are not using a Laravel starter kit and would like to manually configure SSR, we'll first create an SSR entry point file within our Laravel project.
...
Next, we need to update our Vite configuration to build our new SSR entry point. We can do this by adding a `ssr` property to Laravel's Vite plugin configuration in our `vite.config.js` file.
...
Next, let's update the `build` script in our `package.json` file to also build our SSR entry point.
